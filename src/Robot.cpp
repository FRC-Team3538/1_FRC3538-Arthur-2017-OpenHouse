...
class Robot: public IterativeRobot  // FRC3538 IRI 2016
{
	...
	// Chooser setup
	frc::SendableChooser<std::string> chooser, Def_pos, DebugDisplay, CamMenu, autoDelay;
	//frc::SendableChooser<std::string> chooser, Def_pos, DebugDisplay, CamMenu, autoDelay;
...
private:
	...
	//	AutoLow = prefs->GetBoolean("AutoLow");
		...
		chooser.AddObject(autoOuterA1, autoOuterA1);
		chooser.AddObject(autoOuterA2, autoOuterA2);
		chooser.AddObject(autoOuterB1, autoOuterB1);
		chooser.AddObject(autoOuterB2, autoOuterB2);
		chooser.AddObject(autoOuterB3, autoOuterB3);
		chooser.AddObject(autoOuterD1, autoOuterD1);
		chooser.AddObject(autoOuterD2, autoOuterD2);
		chooser.AddObject(autoOuterE0, autoOuterE0);
		frc::SmartDashboard::PutData("Auto Outer6", &chooser);

		//Def_pos = new SendableChooser();
		Def_pos.AddDefault(autoDefDefault, autoDefDefault);
		Def_pos.AddObject(autoDef1, autoDef1);
		Def_pos.AddObject(autoDef2, autoDef2);
		Def_pos.AddObject(autoDef3, autoDef3);
		Def_pos.AddObject(autoDef4, autoDef4);
		Def_pos.AddObject(autoDef5, autoDef5);
		frc::SmartDashboard::PutData("Auto Def Position3", &Def_pos);

		//DebugDisplay = new SendableChooser();
		DebugDisplay.AddDefault(DebugDefault, DebugDefault);
		DebugDisplay.AddObject(DebugSet1, DebugSet1);
		frc::SmartDashboard::PutData("Debug Menu", &DebugDisplay);

		//CamMenu = new SendableChooser();
		CamMenu.AddDefault(CamDefault, CamDefault);
		CamMenu.AddObject(CamOn, CamOn);
		frc::SmartDashboard::PutData("Camera Menu", &CamMenu);

		//autoDelay = new SendableChooser();
		autoDelay.AddDefault(autoDelayDefault, autoDelayDefault);
		autoDelay.AddObject(autoDelay1, autoDelay1);
		autoDelay.AddObject(autoDelay2, autoDelay2);
		frc::SmartDashboard::PutData("AutoDelay", &autoDelay);
	...
	void AutonomousInit()
	{
		...
		autoDelaySelected = autoDelay.GetSelected();

		DefSelected = Def_pos.GetSelected();
...
//			AutoDelayTime.Reset();
		...
		DebugSelected = DebugDisplay.GetSelected();
	...
	void TeleopPeriodic()
	{
		...
		SmartDashboard::PutBoolean("Override", Override);  //pull down if up
